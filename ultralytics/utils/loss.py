...
from ultralytics.utils.tal import RotatedTaskAlignedAssigner, TaskAlignedAssigner, dist2bbox, dist2rbox, make_anchors, \
    TaskAlignedAssigner_ForRetainLoss
...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()  # 预测分数
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()  # 预测分布
        ...
        # gt_labels [N, numbers, 1]
        # gt_bboxes [N, numbers, 4]
        # mask_gt [N, numbers, 1]
        mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0)
        # Pboxes
        pred_bboxes = self.bbox_decode(anchor_points, pred_distri)  # xyxy, (b, h*w, 4) #  对bbox解码

        # pred_scores [2, 8400, 3]
...
class v8OBBLoss(v8DetectionLoss):
    ...
    def __call__(self, preds, batch):
        ...
        # import pdb
        # pdb.set_trace()
        # feats[0].shape -> torch.Size([b, 70, 80, 80])
        # feats[1].shape -> torch.Size([b, 70, 40, 40])
        # feats[2].shape -> torch.Size([b, 70, 20, 20])
        # pred_angle.shape -> torch.Size([b, 1, 8400])
        batch_size = pred_angle.shape[0]  # batch size, number of masks, mask height, mask width
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc), 1)
        # pred_distri.shape -> torch.Size([b, 64, 8400])
        # pred_scores.shape -> torch.Size([b, 6, 8400]) # 6 is the number of classes
        # b, grids, ..
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()
        pred_angle = pred_angle.permute(0, 2, 1).contiguous()

        # pred_distri.shape -> torch.Size([b, 8400, 64])
        # pred_scores.shape -> torch.Size([b, 8400, 6]) # 6 is the number of classes
        # pred_angle.shape -> torch.Size([b, 8400, 1])

        dtype = pred_scores.dtype
        imgsz = torch.tensor(feats[0].shape[2:], device=self.device, dtype=dtype) * self.stride[0]  # image size (h,w)
        anchor_points, stride_tensor = make_anchors(feats, self.stride, 0.5)
        # anchor_points.shape -> torch.Size([8400, 2])
        # stride_tensor.shape -> torch.Size([8400, 1])
        ...
        try:
            ...
            # targets.shape -> torch.Size([119, 7])


            rw, rh = targets[:, 4] * imgsz[0].item(), targets[:, 5] * imgsz[1].item()
            # rw.shape -> torch.Size([119])
            # rh.shape -> torch.Size([119])
