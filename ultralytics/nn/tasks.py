...
class DetectionModel(BaseModel):
    ...
    #         m.inplace = self.inplace
    #         forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB)) else self.forward(x)
    #         m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))])[0:3]  # forward
    #         m.bias_init()  # only run once
    #
    #     else:
    #         self.stride = torch.Tensor([32])  # default stride for i.e. RTDETR
    ...
    #         m.inplace = self.inplace
    #         forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB)) else self.forward(x)
    #         m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))])[3:6]  # forward
    #         m.bias_init()  # only run once
    #     else:
    #         self.stride = torch.Tensor([32])  # default stride for i.e. RTDETR
    #
    #     m = self.model[-4]  # Detect()
    #     if isinstance(m, (Detect, Segment, Pose, OBB)):
    #         s = 256  # 2x min stride
    #         m.inplace = self.inplace
    #         forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB)) else self.forward(x)
    #         m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))])[6:9]  # forward
    #
    #         self.stride = m.stride
    #         m.bias_init()  # only run once
    #     else:
