...
class PairedMosaic(BaseMixTransform):
    """
    PairedMosaic augmentation.
    ...
    def _mosaic3(self, labels):
        ...
        for i in range(3):
            ...
            if i == 0:  # center
                img3 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                img3_lwir = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # right
                c = s + w0, s, s + w0 + w, s + h
            ...
            img3[y1:y2, x1:x2] = img[y1 - padh :, x1 - padw :]  # img3[ymin:ymax, xmin:xmax]
            img3_lwir[y1:y2, x1:x2] = img_lwir[y1 - padh:, x1 - padw:]  # img3[ymin:ymax, xmin:xmax]
            # hp, wp = h, w  # height, width previous for next iteration
            ...
            mosaic_labels.append(labels_patch)
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img"] = np.concatenate((img3[-self.border[0]: self.border[0], -self.border[1]: self.border[1]],
                                              img3_lwir[-self.border[0]: self.border[0],
                                              -self.border[1]: self.border[1]]), axis=2)
        # final_labels["img"] = img3[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        # final_labels["img_lwir"] = img3_lwir[-self.border[0]: self.border[0], -self.border[1]: self.border[1]]
        return final_labels
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img4_lwir = np.full((s * 2, s * 2, img_lwir.shape[2]), 114, dtype=np.uint8)
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
                x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
            elif i == 1:  # top right
                x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
            ...
            mosaic_labels.append(labels_patch)
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img"] = np.concatenate((img4, img4_lwir), axis=2)
        return final_labels
    ...
    def _mosaic9(self, labels):
        ...
        for i in range(9):
            ...
            if i == 0:  # center
                img9 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img9_lwir = np.full((s * 3, s * 3, img_lwir.shape[2]), 114, dtype=np.uint8)
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
            ...
            # Image
            img9[y1:y2, x1:x2] = img[y1 - padh :, x1 - padw :]  # img9[ymin:ymax, xmin:xmax]
            img9_lwir[y1:y2, x1:x2] = img_lwir[y1 - padh:, x1 - padw:]
            hp, wp = h, w  # height, width previous for next iteration
            ...
            mosaic_labels.append(labels_patch)
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img"] = np.concatenate((img9[-self.border[0]: self.border[0], -self.border[1]: self.border[1]],
                                              img9_lwir[-self.border[0]: self.border[0],
                                              -self.border[1]: self.border[1]]), axis=2)
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        final_labels = {
            "im_file": mosaic_labels[0]["im_file"],
            # "im_file_lwir": mosaic_labels[0]["im_file_lwir"],
            "ori_shape": mosaic_labels[0]["ori_shape"],
            "resized_shape": (imgsz, imgsz),
            "cls": np.concatenate(cls, 0),
            "instances": Instances.concatenate(instances, axis=0),
...
class PairedRandomPerspective(RandomPerspective):
    ...
    def affine_transform(self, img,img_lwir, border):
        """
        Applies a sequence of affine transformations centered around the image center.
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                img = cv2.warpPerspective(img, M, dsize=self.size, borderValue=(114, 114, 114))
                img_lwir = cv2.warpPerspective(img_lwir, M, dsize=self.size, borderValue=(114, 114, 114))
            else:  # affine
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                img_lwir = cv2.warpAffine(img_lwir, M[:2], dsize=self.size, borderValue=(114, 114, 114))
        return img,img_lwir, M, s
    ...
    def __call__(self, labels):
        ...
        # M is affine matrix
        # Scale for func:`box_candidates`
        img,img_lwir, M, scale = self.affine_transform(img,img_lwir, border)
...
class PairedRandomHSV(RandomHSV):
    """
    This class is responsible for performing random adjustments to the Hue, Saturation, and Value (HSV) channels of an
    image.
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            hue_lwir, sat_lwir, val_lwir = cv2.split(cv2.cvtColor(img_lwir, cv2.COLOR_BGR2HSV))
            dtype = img_lwir.dtype  # uint8

            x = np.arange(0, 256, dtype=r.dtype)
            lut_hue_lwir = ((x * r[0]) % 180).astype(dtype)
            lut_sat_lwir = np.clip(x * r[1], 0, 255).astype(dtype)
            lut_val_lwir = np.clip(x * r[2], 0, 255).astype(dtype)

            im_hsv_lwir = cv2.merge((cv2.LUT(hue_lwir, lut_hue_lwir), cv2.LUT(sat_lwir, lut_sat_lwir), cv2.LUT(val_lwir, lut_val_lwir)))
            cv2.cvtColor(im_hsv_lwir, cv2.COLOR_HSV2BGR, dst=img_lwir)  # no return needed
    ...
    def __call__(self, labels):
        ...
        if self.direction == "horizontal" and random.random() < self.p:
            ...
            if self.flip_idx is not None and instances.keypoints is not None:
                instances.keypoints = np.ascontiguousarray(instances.keypoints[:, self.flip_idx, :])
        # labels["img"] = np.ascontiguousarray(img)
        labels["img"] = np.concatenate((np.ascontiguousarray(img), np.ascontiguousarray(img_lwir)), axis=2)

        # labels["img_lwir"] = np.ascontiguousarray(img_lwir)
        labels["instances"] = instances
        return labels
...
class PairedLetterBox(LetterBox):
    ...
    def __call__(self, labels=None, image=None):
        ...
        # img = labels.get("img") if image is None else image
        # img_lwir = labels.get("img_lwir") if image is None else image
        shape = img.shape[:2]  # current shape [height, width]
        new_shape = labels.pop("rect_shape", self.new_shape)
        if isinstance(new_shape, int):
            new_shape = (new_shape, new_shape)
        ...
        if shape[::-1] != new_unpad:  # resize
            img = cv2.resize(img, new_unpad, interpolation=cv2.INTER_LINEAR)
            img_lwir = cv2.resize(img_lwir, new_unpad, interpolation=cv2.INTER_LINEAR)
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        img = cv2.copyMakeBorder(
            img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
        img_lwir = cv2.copyMakeBorder(
            img_lwir, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
        if labels.get("ratio_pad"):
            labels["ratio_pad"] = (labels["ratio_pad"], (left, top))  # for evaluation
        # print(img.shape,img_lwir.shape)
        if len(labels):
            # import pdb
            # pdb.set_trace()
            labels = self._update_labels(labels, ratio, dw, dh)
...
class PairedCopyPaste(CopyPaste):
    """
    Implements the Copy-Paste augmentation as described in the paper https://arxiv.org/abs/2012.07177. This class is
    responsible for applying the Copy-Paste augmentation on images and their corresponding instances.
    """
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            _, w, _ = im.shape  # height, width, channels
            im_new = np.zeros(im.shape, np.uint8)
            im_new_lwir = np.zeros(im_lwir.shape, np.uint8)
            ...
            i = cv2.flip(im_new, 1).astype(bool)
            i_lwir = cv2.flip(im_new_lwir, 1).astype(bool)
            im[i] = result[i]
            im_lwir[i_lwir] = result_lwir[i_lwir]

        labels["img"] = np.concatenate((im, im_lwir), axis=2)
...
class PairedAlbumentations(Albumentations):
    """
    Albumentations transformations.
...
class PairedFormat:
    ...
    def __call__(self, labels):
        ...
        if self.return_mask:
            ...
            labels["masks"] = masks
        labels["img"] = self._format_img(img)
        labels["img_lwir"] = self._format_img(img_lwir)
        labels["cls"] = torch.from_numpy(cls) if nl else torch.zeros(nl)
        labels["bboxes"] = torch.from_numpy(instances.bboxes) if nl else torch.zeros((nl, 4))
        if self.return_keypoint:
            labels["keypoints"] = torch.from_numpy(instances.keypoints)
    ...
    pre_transform = Compose(
        [
            ...
            PairedRandomPerspective(
                ...
                shear=hyp.shear,
                perspective=hyp.perspective,
                pre_transform=None if stretch else PairedLetterBox(new_shape=(imgsz, imgsz)),
            ),
        ]
    )
    flip_idx = dataset.data.get("flip_idx", [])  # for keypoints augmentation
